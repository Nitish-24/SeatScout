...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleToggleEmail = (enabled: boolean) => {
    ...
    try {
      fetch('/api/notifications/email/save-preference', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          enabled,
          format: updated.emailAlertFormat || 'detailed'
        })
      }).catch(() => {});
    } catch {}
  };

  const handleEmailAddressChange = (val: string) => {
    ...
    // Sync preference with server if valid
      ...
      try {
        fetch('/api/notifications/email/save-preference', {
          ...
          body: JSON.stringify({
            ...
            format: settings.emailAlertFormat || 'detailed'
          })
        }).catch(() => {});
      } catch {}
    }
  ...
  const handleFormatChange = (format: 'detailed' | 'summary') => {
    const updated = {
      ...settings,
      emailAlertFormat: format
    };
    onUpdateSettings(updated);
  ...
  const handleSendTestEmail = async () => {
    const targetEmail = settings.emailAddress?.trim();
    if (!targetEmail || !isValidEmail(targetEmail)) {
      setEmailInputError('Please enter a valid email address first (e.g. name@example.com)');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setEmailTestResult({
          success: true,
          message: data.message || `Test long-form berth dossier dispatched to ${targetEmail}!`
        });
      } else {
        setEmailTestResult({
          success: false,
      ...
      setEmailTestResult({
        success: false,
        message: err.message || 'Network error while contacting email gateway.'
      });
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto py-4 space-y-6">
      ...
      {/* Long-Form Email Alerts Section (For Offline / Away from Screen Monitoring) */}
      <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <Mail className="w-5 h-5 text-indigo-400" />
            <div>
              <h3 className="text-base font-bold text-white">Long-Form Email Seat Alerts</h3>
              <p className="text-xs text-slate-400">
                Receive comprehensive seat dossiers straight to your inbox with direct IRCTC booking links and charting schedules when you aren't actively monitoring the browser.
              </p>
            </div>
          </div>
          <label className="flex items-center space-x-2 cursor-pointer text-xs">
            ...
            />
            <span className="text-slate-300 font-semibold">Enable Email Alerts</span>
          </label>
        </div>
          ...
          <div className="space-y-4 pt-2 border-t border-slate-800/80 animate-in fade-in duration-200">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-950 border text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-2 transition-all ${
                    emailInputError 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                <button
                  ...
                  className={`p-3 rounded-xl border text-left text-xs transition-all cursor-pointer ${
                    (settings.emailAlertFormat || 'detailed') === 'detailed'
                      ? 'bg-indigo-500/15 border-indigo-500/60 text-indigo-200 shadow-sm'
                      : 'bg-slate-950 border-slate-800 text-slate-400 hover:text-slate-300'
                  }`}
                >
                  <div className="font-bold flex items-center space-x-1.5">
                    <FileText className="w-3.5 h-3.5 text-indigo-400" />
                    <span>Comprehensive Berth Dossier (Recommended)</span>
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">
                    Includes direct 1-click IRCTC booking link, quota guidance, charting timeline, and coach recommendations.
                  </p>
                </button>
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border text-left text-xs transition-all cursor-pointer ${
                    settings.emailAlertFormat === 'summary'
                      ? 'bg-indigo-500/15 border-indigo-500/60 text-indigo-200 shadow-sm'
                      : 'bg-slate-950 border-slate-800 text-slate-400 hover:text-slate-300'
                  }`}
                >
                  <div className="font-bold flex items-center space-x-1.5">
                    ...
                    <span>Compact Alert Summary</span>
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">
                    Lightweight notification showing train number, available seat count, and direct booking URL.
                  </p>
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold text-xs shadow-md transition-all flex items-center space-x-2 cursor-pointer"
              >
                {emailSending ? (
                  <>
                  ...
                  <>
                    ...
                    <span>Send Test Long-Form Email Alert</span>
                  </>
                )}
              </button>

              <span className="text-[11px] text-slate-400">
                Dispatches immediately so you can verify your inbox formatting.
              </span>
            </div>
            ...
            {emailTestResult && (
              <div
                className={`p-3.5 rounded-xl text-xs flex items-start space-x-2.5 animate-in fade-in duration-200 border ${
              ...
              >
                ...
                <div className="space-y-0.5">
                  <div className="font-semibold">{emailTestResult.message}</div>
                  {emailTestResult.success && (
                    <div className="text-[11px] text-emerald-300/80">
                      Check your inbox or spam folder for "🚨 SeatScout Long-Form Dossier".
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 text-[11px] text-slate-400 space-y-1">
              <div className="flex items-center space-x-1.5 text-slate-300 font-semibold">
                ...
                <span>Why Long-Form Email Alerts?</span>
              </div>
              <p>
                Unlike ephemeral push chimes that vanish if you're away from your computer, long-form email alerts preserve the full booking link, train schedule, and quota regulations indefinitely in your inbox.
              </p>
            </div>
          </div>
        ) : (
          <div className="p-3 rounded-xl bg-slate-950/50 border border-slate-800/80 text-xs text-slate-400 flex items-center justify-between">
            <span>Email alerts are currently turned off. Toggle above to receive alerts when not at your desk.</span>
          </div>
        )}
      </div>
